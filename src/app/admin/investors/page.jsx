...
import { getInvestorList } from "../services/investors"
...
export default function InvestorsPage() {
  ...
  const fetchInvestors = async ({ page = 1, searchVal = "", from = "", to = "" }) => {
    ...
    setInvestors(res.data?.investor_list ?? [])
    setTotalPages(res.data?.pagination?.total_pages ?? 1)
    setTotalCount(res.data?.pagination?.total ?? 0)
    return res
  }

  // Only show toast on failure — success just displays data silently
  ...
  const handlePageChange = (page) => {
    setCurrentPage(page)
    fetchInvestors({ page, searchVal: search, from: fromDate, to: toDate })
      .then(showErrorToast)
      .catch(() => {})
  }
  ...
  const handleSearch = (val) => {
    setSearch(val)
    setCurrentPage(1)
    fetchInvestors({ page: 1, searchVal: val, from: fromDate, to: toDate })
      .then(showErrorToast)
      .catch(() => {})
  }
  ...
  const handleDateChange = (range) => {
    ...
    setToDate(to)
    setCurrentPage(1)
    fetchInvestors({ page: 1, searchVal: search, from, to })
      .then(showErrorToast)
      .catch(() => {})
  }
  ...
  return (
    <section className="space-y-6">
      <header className="flex items-start justify-between gap-4">
        <section>
          <h2 className="text-2xl font-bold text-customBlack mb-1">Investors</h2>
          <p className="text-sm text-grey">Showing all investors</p>
        </section>
          ...
          <Table>
            ...
            <TableBody className="bg-white">
              {investors.length === 0 ? (
                ...
                investors.map((inv, i) => (
                  <TableRow key={inv.bvn + i}>
                    <TableCell className="px-6 py-6 text-tableGrey whitespace-nowrap">{inv.investor_name}</TableCell>
                    <TableCell className="px-6 py-6 text-tableGrey whitespace-nowrap">{inv.bvn}</TableCell>
                    <TableCell className="px-6 py-6 text-tableGrey whitespace-nowrap">{inv.email_address}</TableCell>
                    <TableCell className="px-6 py-6 text-tableGrey whitespace-nowrap">{inv.type}</TableCell>
                    <TableCell className="px-6 py-6 text-tableGrey whitespace-nowrap">{fmtDate(inv.date_joined)}</TableCell>
                    <TableCell className="px-6 py-6 whitespace-nowrap">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${
                        inv.status === "Active" ? "bg-lightGreen text-[#16A34A]" : "bg-[#FEF3C7] text-[#D97706]"
                      }`}>
                        {inv.status}
                      </span>
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="min-w-48">
                          ...
                          <DropdownMenuItem
                            ...
                            onClick={() => router.push(`/admin/investors/${inv.bvn}`)}
                          >
                            View User Details
                          </DropdownMenuItem>
                        </DropdownMenuContent>
